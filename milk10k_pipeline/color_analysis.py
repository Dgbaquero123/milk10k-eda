"""
color_analysis.py
Parte 2: histogramas y estadisticas de color a nivel dataset.
"""
...
def sample_paths_per_class(df, images_dir, n_per_class=25, seed=0):
    """Devuelve dict {clase: [paths disponibles]} con n por clase."""
...
def per_image_color_stats(paths_by_class):
    """Devuelve DataFrame con mean/std por canal y por clase."""
    rows = []
    for cls, paths in paths_by_class.items():
        for p in paths:
            try:
...
def plot_average_gray_histogram(hists, out_path):
    ...
    ax.set_title("Histograma gris promedio por clase")
    ax.legend()
    plt.tight_layout()
...
def plot_average_rgb_histogram(hists, out_path):
    ...
    for ax, (cls, h) in zip(axes, hists.items()):
        for c in range(3):
            ax.plot(np.linspace(0, 255, h.shape[1]), h[c], color=colors[c], label=["R", "G", "B"][c])
        ax.set_title(cls)
...
def plot_color_stats_boxplots(df_stats, out_path):
    ...
    for i, ch in enumerate(["R", "G", "B"]):
        ...
        axes[0, i].set_title(f"{ch} mean por clase")
        axes[0, i].set_xlabel("")
        df_stats.boxplot(column=f"{ch}_std", by="class", ax=axes[1, i])
        axes[1, i].set_title(f"{ch} std por clase")
        axes[1, i].set_xlabel("")
    plt.suptitle("")
    plt.tight_layout()
    plt.savefig(out_path, dpi=120)
...
def run_part2(df, images_dir, figures_dir, n_per_class=25, seed=0):
    ...
    paths_by_class = sample_paths_per_class(df, images_dir, n_per_class=n_per_class, seed=seed)
    ...
    return {"paths_by_class": paths_by_class, "gray_hists": gray_h, "rgb_hists": rgb_h, "stats_df": stats_df}
